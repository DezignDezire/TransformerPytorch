...
class Transformer(nn.Module):
    ...
    def forward(self, src, trg):
        ...
        embed_src = self.dropout(
            (self.src_word_embedding(src) + self.src_positional_embedding(src_position))
        )

        embed_trg = self.dropout(
            (self.trg_word_embedding(trg) + self.trg_positional_embedding(trg_position))
        )
...
train_iterator, valid_iterator, test_iterator = BucketIterator = BucketIterator.splits(
...
sentence = "sollte ich mir ernsthaft Sorgen um meine Freitzeit machen?"
...
for epoch in range(num_epochs):
    ...
    for batch_idx, batch in enumerate(train_iterator):
        ...
        # forward prop (want to predict next output target)
